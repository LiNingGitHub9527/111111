...
import { PeopleNums, Reservation, Reservations } from '../datatypes';
...
const ReservationCalendarModal: FC<Props> = ({
  ...
  const initialSelectedPeopleNums = useMemo(
    (): PeopleNums =>
  ...
  const [selectedPeopleNums, setSelectedPeopleNums] = useState<PeopleNums>(
    initialSelectedPeopleNums
  );
  ...
  const createReservationBlockClickHandler = useCallback(
      ...
      (_event: React.MouseEvent<HTMLDivElement>): void => {
        ...
        if (isMultiple) {
          ...
          if (prevIsSelected) {
            const {
              // eslint-disable-next-line @typescript-eslint/no-unused-vars
              [block.reservation_block_token]: _removedKey,
              ...nextSelectedPeoplesNum
            } = selectedPeopleNums;
            setSelectedPeopleNums(nextSelectedPeoplesNum);
          } else {
            setSelectedPeopleNums({
              ...selectedPeopleNums,
    ...
    [activeDate, isMultiple, selectedPeopleNums, selectedReservationBlocks]
  );

  const createPeopleNumPlusHandler = useCallback(
    (block: ReservationBlock) =>
      (_event: React.MouseEvent<HTMLButtonElement>): void => {
        const prevNum = selectedPeopleNums[block.reservation_block_token] ?? 0;
        if (prevNum < block.person_capacity) {
          setSelectedPeopleNums({
            ...selectedPeopleNums,
            [block.reservation_block_token]: prevNum + 1,
          });
        }
      },
    [selectedPeopleNums]
  ...
  const createPeopleNumMinusHandler = useCallback(
    (block: ReservationBlock) =>
      (_event: React.MouseEvent<HTMLButtonElement>): void => {
        const prevNum = selectedPeopleNums[block.reservation_block_token] ?? 0;
        if (prevNum > 1) {
  ...
  const selectedReservations = useMemo(
      ...
      Object.entries(selectedReservationBlocks).reduce((acc, [key, value]) => {
        ...
        const selectedReservationsByDate = value
          .reduce((a: Reservation[], c: ReservationBlock): Reservation[] => {
            const peopleNum =
              selectedPeopleNums[c.reservation_block_token] ?? 0;
            if (peopleNum > 0 && peopleNum <= c.person_capacity) {
              a.push({
                block: c,
                peopleNum,
              });
    ...
    [selectedPeopleNums, selectedReservationBlocks]
  );
  ...
  const isDirty = useMemo((): boolean => {
    ...
    for (const key of selectedKeys) {
      ...
      const isChanged = selectedValue.some((v: Reservation) => {
        ...
        if (v.peopleNum !== correspondingBlock.peopleNum) {
          return true;
        }
                    ...
                    (block: ReservationBlock, index: number) => (
                      ...
                      >
                        ...
                        {selectedReservationBlocksShown.some(
                          ...
                          >
                            <div>
                              <div className="text-sm">ご利用人数</div>
                            </div>
                            <div className="flex items-center justify-between w-28">
                              <button
                                ...
                                onClick={createPeopleNumMinusHandler(block)}
                              >
                                <MinusCircleIcon
                                  className={[
                                    'w-8 h-8',
                              ...
                              <button
                                ...
                                disabled={
                                  selectedPeopleNums[
                                    block.reservation_block_token
                                  ] >= block.person_capacity
                                }
                                className="rounded-full"
                                onClick={createPeopleNumPlusHandler(block)}
                              >
                                <PlusCircleIcon
                                  className={[
                                    'w-8 h-8',
